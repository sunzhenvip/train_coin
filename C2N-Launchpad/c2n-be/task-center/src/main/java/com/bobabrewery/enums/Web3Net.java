...
public enum Web3Net {
    ...
    public static Web3j valueOfChainId(Integer chainId) {
        Web3Net[] values = Web3Net.values();
        for (Web3Net value : values) {
            if (value.getChainId().equals(chainId)) {
                return value.getWeb3j();
            }
        }
        throw new RuntimeException("NOT FOUND NETWORK");
    }
